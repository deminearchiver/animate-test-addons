...
#define OPTIONAL(method_ptr, fallback, ...) (method_ptr ? ((*method_ptr)(__VA_ARGS__)) : fallback)
...
namespace animate {
    namespace jsapi {
        ...
        inline Boolean Register(const wchar_t* name, APIMethod method, unsigned int argc) {
            return OPTIONAL(env.DefineFunction, false, env.library, WIDE_TO_SHORT(name), method, argc);
        }
    }
}
...
extern void init_function(animate::jsapi::Environment env); \
extern "C" __declspec(dllexport) void MM_InitWrapper(animate::jsapi::Environment* external, unsigned int env_size) { \
    ...
    return init_function(animate::jsapi::env); \
}
